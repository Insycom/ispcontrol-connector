...
export class MachineClient {
  ...
  static async enroll(
    ...
    const result = (await parseJsonOrThrow(response)) as {
      connectorId: string;
      accessToken: string;
      refreshToken: string;
  ...
  async configuration(): Promise<MonitoredDeviceConfig[]> {
    ...
    return (await parseJsonOrThrow(response)) as MonitoredDeviceConfig[];
  }
  ...
  async sendEvents(events: OutboxEvent[]): Promise<string[]> {
    ...
    return ((await parseJsonOrThrow(response)) as { accepted: string[] }).accepted;
  }
  ...
  async claimJobs(): Promise<MachineJob[]> {
    ...
    return (await parseJsonOrThrow(response)) as MachineJob[];
  }
  ...
  private async refresh(): Promise<void> {
    ...
    const tokens = (await parseJsonOrThrow(response)) as {
      accessToken: string;
      refreshToken: string;
    };
...
async function parseJsonOrThrow(response: Response): Promise<unknown> {
  ...
  try {
    ...
    throw new Error(`Invalid JSON from ${response.url || "connector API"}`);
  }
}
